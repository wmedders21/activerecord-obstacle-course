...
# ----------------------
  ...
  it '27. returns a table of information for all users orders' do
    ...
    # ------------------ ActiveRecord Solution ----------------------
    custom_results = User.joins(:orders).select('users.name, COUNT(orders.*) AS total_order_count').group('users.id').order('total_order_count')
    # ---------------------------------------------------------------
  ...
  it '28. returns a table of information for all users items' do
    ...
    # ------------------ ActiveRecord Solution ----------------------
    custom_results = User.joins(orders: :order_items).select('users.name, COUNT(order_items.*) AS total_item_count').group('users.id').order('users.name')
    # ---------------------------------------------------------------
  ...
  it '29. returns a table of information for all users orders and item counts' do
    ...
    # ------------------ ActiveRecord Solution ----------------------
    data = User.select('users.name AS user_name, orders.id AS order_id, FLOOR(orders.amount / COUNT(order_items.id)) AS avg_item_cost').joins(orders: :order_items).group('users.name, orders.id').order('user_name DESC, avg_item_cost')
    # ---------------------------------------------------------------
